...
<head>
	...
	<title>Centre Dent'R Expert - Effacer un client</title>
			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					if(isset($_POST["id"]) && !empty($_POST["id"])){
						...
						$sql3 = "DELETE FROM patient  WHERE id = $idp";

						// check if insertion was succesfully done
						if(mysqli_query($conn, $sql3) === TRUE){
							//echo "<h3 class='error-msg'> le patient a été effacé  avec succès.</h3>" ;
						} else{
						...
						if(mysqli_query($conn, $sql2) === TRUE){
							...
							echo "<h3 class='success-msg'>le client a été effacé  avec succès.</h3>" ;
							echo "<meta http-equiv='refresh' content='0;url=client.php'> ";
						...
						while($row = $result->fetch_assoc()) { ?>
							...
							<div class="card shadow mb-4">
								<div class="card-header py-3">
									<h6 class="m-0 font-weight-bold text-primary"> Etez-vous sûr de vouloir effacer  le patient : <?php  echo $row["prenom"] ?>	<?php  echo $row["nom"] ?></h6>
								</div>
								<div class="card-body">
									...
									<form name="sign-up" action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" class="user view"
										...
										<div class="form-group row">
											...
											<div class="col-sm-6">
												...
												<a href="client.php" class="btn btn-secondary btn-user btn-block">Annuler</a>
											</div>
										</div>
