...
<!-- Page Heading -->
<h1 class="h3 mb-2 text-gray-800">Prendre un rendez-vous</h1>
...
if ($result->num_rows > 0) {
	...
	while($row = $result->fetch_assoc()) { ?>
		...
		<div class="card shadow mb-4">
			...
			<div class="row">
			<div class="col-8"><h6 class="m-0 font-weight-bold text-primary">Profil utilisateur: <?php  echo $row["prenom"] ?>	<?php  echo $row["nom"] ?></h6></div>
				  <div class="col-4"> </div>
				</div>
...
<!-- /.container-fluid -->
				...
				if(!empty($param_id_profil)){
						$param_id2 = $param_id_profil;
											

						}
						$conn = OpenCon();
						$sql = "SELECT id, id_utilisateur  FROM patient WHERE id_utilisateur  = $param_id2" ;
						...
						if ($result->num_rows > 0) {
							...
							while($row = $result->fetch_assoc()) { ?>
									...
									<?php  $idpprofil = $row['id'] ?>
										<div class="container-fluid">
										<h3 class="h3 mb-2 text-gray-800 ">Prendre un rendez-vous</h3>
										...
										<div class="container-fluid">
													...
													if($_SERVER["REQUEST_METHOD"] == "POST"){
														...
														VALUES ($idpprofil,'$date','$type',$succursale,'$commentaire')";
														
														// check if insertion was succesfully done
														if(mysqli_query($conn, $sql3) === TRUE){
															echo "<h3 class='success-msg'>Le rendez-vous a été ajouté dans le système avec succès.</h3>" ;  ?> 
				...
				<div class="container-fluid deja">
					...
					<!-- Page Heading -->
					<h1 class="h3 mb-2 text-gray-800">Completez votre profil avant de prendre un rendez-vous</h1>
					...
					if($_SERVER["REQUEST_METHOD"] == "POST"){
						...
						// check if insertion was succesfully done
						if(mysqli_query($conn, $sql3) === TRUE){
							echo "<meta http-equiv='refresh' content='0;url=rendezvous-client.php'> ";  ?> 
...
<?php } else{
					...
					<form name="sign-up" action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" class="user"
						...
						<fieldset class="border p-2">
							<legend  class="w-auto">Dossier</legend>

							<div class="form-group row">
								<div class="col-sm-6 mb-3 mb-sm-0">
								<div class="form-label" style="margin-bottom:-12px; margin-top:-10px; ">
						...
						<div class="form-group row">
							<div class="col-sm-12">
								<input class="btn btn-primary btn-user btn-block" type="submit" name="signup-btn"
								id="signup-btn" value="Completez le profil">
							</div>
						</div>
					</form>
